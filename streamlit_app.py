...
if page == "🏠 仪表板":
    ...
    with col_a:
        ...
        if has_model:
            ...
            pos = sum(1 for r in feedback if r.get("label") == 1)
            neg = sum(1 for r in feedback if r.get("label") == -1)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not pool:
        ...
        def _do_label(label: int, enqueue: bool = False):
            record_feedback(article, label)
            _read_feedback.clear()
            if enqueue:
        ...
        for i, item in enumerate(queue):
            with st.container(border=True):
                ...
                with col_t:
                    ...
                    st.caption(f"{item.get('source','—')} · `{item.get('article_id','')}`")
                    url = item.get("url", "")
                    if url:
                ...
                with col_del:
                    if st.button("✕", key=f"del_{i}"):
                        remove_from_payload_queue(item.get("article_id", ""))
                        st.rerun()
    else:
        ...
        if results:
            for a in results:
                ...
                with col_add:
                    ...
                    if any(x.get("article_id") == aid for x in queue):
                        st.caption("已在")
                    elif st.button("＋", key=f"add_{aid}"):
                        add_to_payload_queue(a)
                        st.rerun()
        else:
            st.info("无匹配结果")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with col_opts:
        st.subheader("运行选项")
        opt_nomail  = st.checkbox("🔕 不发邮件 (--no-mail)", value=True)
        opt_chinese = st.checkbox("🇨🇳 中文翻译 (--chinese)")
        opt_force   = st.checkbox("⚡ 强制执行 (--force)", value=False)
    with col_run:
        st.subheader("执行")
    ...
    st.divider()
    st.subheader("⏰ 定时任务")
    st.info("每天 10:00 自动运行（默认不发邮件，并遵守标记门槛）")
