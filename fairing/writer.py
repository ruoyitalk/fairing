...
def _build_obsidian_lines(articles: list[dict], today_str: str, use_zh: bool = False) -> list[str]:
    sources = list(dict.fromkeys(a["source"] for a in articles))
    categories = list(dict.fromkeys(a["category"] for a in articles))
    tags = ["daily-digest"] + [c.lower().replace(" ", "-").replace("/", "-") for c in categories]
    ...
    lines: list[str] = [
        ...
        "",
        f"> {len(articles)} articles from {len(sources)} sources",
        "",
    ]
