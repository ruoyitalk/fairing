...
from .paths import seen_urls_file as _seen_urls_file
RETAIN_DAYS = 30
...
def _load() -> dict:
    if _seen_urls_file().exists():
        try:
            raw = json.loads(_seen_urls_file().read_text(encoding="utf-8"))
...
def _save(state: dict) -> None:
    _seen_urls_file().write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")
...
def mark_seen(articles: list[dict]) -> None:
    ...
    cutoff = (datetime.now(_TZ_BEIJING).date() - timedelta(days=RETAIN_DAYS)).isoformat()
